...
export const CategoryProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchCategories = async () => {
    ...
    } catch (error: unknown) { // Bắt lỗi tổng quát hơn
      console.error('Failed to fetch categories:', error);
      if (error instanceof Error) {
        alert(`Không thể tải danh mục. Vui lòng kiểm tra kết nối API: ${error.message}`);
      } else {
        alert('Không thể tải danh mục. Vui lòng kiểm tra kết nối API.');
      }
  ...
  const addCategory = async (newCatData: { name: string }) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        alert('Lỗi khi thêm danh mục. Vui lòng kiểm tra kết nối API.');
      }
    } finally {
      setIsLoadingCategories(false);
    }
  ...
  const updateCategory = async (id: number, updatedCatData: { name: string }) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        alert('Lỗi khi cập nhật danh mục. Vui lòng kiểm tra kết nối API.');
      }
    } finally {
      setIsLoadingCategories(false);
    }
  ...
  const deleteCategory = async (id: number) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        alert('Lỗi khi xóa danh mục. Vui lòng kiểm tra kết nối API.');
      }
    } finally {
      setIsLoadingCategories(false);
    }
